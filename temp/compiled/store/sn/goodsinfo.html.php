...
function buy_now(store_id)
{
    ...
    buy_now_add_cart(spec_id, quantity, store_id);
}


/* add buy_now_add_cart */
function buy_now_add_cart(spec_id, quantity, store_id)
{
    var url = SITE_URL + '/index.php?app=cart&act=add';
    $.getJSON(url, {'spec_id':spec_id, 'quantity':quantity}, function(data){
		if (data.done)
        {
			location.href='index.php?app=order&goods=cart&store_id='+store_id;
        }else{
            alert(data.msg);
        }
    });
...
?>
    ...
    <div class="ware_text">
        <div class="rate">
			...
			<span class="letter1">市场价: </span><span><del><?php echo price_format($this->_var['goods']['mk_price']); ?></del></span><br />
...
?><?php echo $this->_var['tag']; ?>&nbsp;&nbsp;&nbsp;<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?><br />
        ...
        <ul class="ware_btn">
            ...
            <li class="btn_c1" title="立刻购买"><a href="javascript:buy_now(<?php echo $this->_var['store']['store_id']; ?>)"></a></li>
