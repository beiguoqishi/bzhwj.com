...
?>
                ...
                <p>
                    ...
                    QQ:
                    <a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin=<?php echo htmlspecialchars($this->_var['store']['im_qq']); ?>&site=qq&menu=yes"><img border="0" src="http://wpa.qq.com/pa?p=2:<?php echo htmlspecialchars($this->_var['store']['im_qq']); ?>:53" alt="点击这里给我发消息" title="点击这里给我发消息"/></a>
        ...
        <div class="module_common">
            ...
            <div class="wrap">
                <div class="wrap_child">
                    <ul class="submenu">
                        <li><span class="all-ctg st-icon none_ico"></span><a <?php if ($this->_var['cur_ctg_id'] == 0): ?>class="cur-ctg"<?php endif; ?> href="<?php echo url('app=store&id=' . $this->_var['store']['store_id']. '&act=search'); ?>">全部商品</a></li>
...
?>
                        ...
                        <li style="padding-left: 15px" class="subment-item">
                            <span class="st-icon none_ico"></span><a <?php if ($this->_var['cur_ctg_id'] == $this->_var['gcategory']['id']): ?>class="cur-ctg"<?php endif; ?> href="<?php echo url('app=store&id=' . $this->_var['store']['store_id']. '&act=search&cate_id=' . $this->_var['gcategory']['id']. ''); ?>"><?php echo htmlspecialchars($this->_var['gcategory']['value']); ?></a>
                            <ul>
...
?>
                                <li><a <?php if ($this->_var['cur_ctg_id'] == $this->_var['child_gcategory']['id']): ?>class="cur-ctg"<?php endif; ?> href="<?php echo url('app=store&id=' . $this->_var['store']['store_id']. '&act=search&cate_id=' . $this->_var['child_gcategory']['id']. ''); ?>"><?php echo htmlspecialchars($this->_var['child_gcategory']['value']); ?></a></li>
                                <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                            </ul>
                        </li>
                        <?php else: ?>
...
<script>
    (function(){
        $('body').on('click','.st-icon',function(e) {
            var $this = $(this);
            $this.toggleClass('none_ico block_ico');
            if ($this.is('.all-ctg')) {
                $this.parents('.submenu').toggleClass('collapse');
            } else {
                $this.parents('li').find('ul').toggleClass('sub-collapse');
            }
        });
    })();
</script>